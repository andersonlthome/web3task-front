...
const CreateTask = ({ data }) => {
  ...
  const handleAuthorizedRoles = (event: any) => {
    ...
    const splittedRoles: readonly bigint[] = authorizedRoles.map((str) =>
      // TODO resolve error Cannot convert adm(string) to a BigInt
      BigInt(str)
    );
    task.authorizedRoles = splittedRoles;
  };

  // provavelmente vai vir do contrato, pois é a role de quem ta criando a task
  ...
  const handleCloseSnackInformation = (
  ...
  return (
    <Stack spacing={2} sx={{ width: "100%" }}>
      <Snackbar
        open={openInformartion}
        autoHideDuration={6000}
        onClose={handleCloseSnackInformation}
      >
        <Alert
          onClose={handleCloseSnackInformation}
          severity="info"
